...
-- Phase 12G Slice 1B: suppress stale/non-analytical valuation and signal fields
-- in the dashboard latest view so old analytical rows cannot look current.
--
-- This migration does NOT change model math, thresholds, provider behavior,
-- or pipeline execution order. It only controls dashboard projection.
--
-- Behavior:
--   - When analysis readiness blocks valuation, valuation fields are NULL.
--   - When analysis readiness blocks signal, signal fields are NULL.
--   - Price, readiness, and data-quality diagnostic lanes remain visible.
...
select distinct on (c.id)
  ...
  case
    when ar.can_run_valuation is false then null
    else v.iv_p25
  end                               as iv_p25,
  case
    when ar.can_run_valuation is false then null
